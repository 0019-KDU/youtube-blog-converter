...
class MongoDBConnectionManager:
    ...
    def _connect(self):
        ...
        try:
            ...
            masked_uri = (
                self._mongodb_uri[:20] + "***" + self._mongodb_uri[-20:]
                if len(self._mongodb_uri) > 40
                else "***"
            )
            ...
            if sys.platform == "win32" and "Invalid syntax" in error_str:
                raise ConnectionError(
                    "MongoDB connection failed due to Windows compatibility issue. Please check your connection string."
                )
            else:
                raise
        finally:
