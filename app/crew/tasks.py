...
def create_tasks(transcriber, writer, youtube_url, language):
    ...
    transcript_task = Task(
        ...
        agent=transcriber,
        callback=lambda task: logger.info(f"Transcript task completed: {task.output[:100] if task.output else 'No output'}...")
    )
    ...
    blog_task = Task(
        ...
        callback=lambda task: logger.info(f"Blog task completed: {len(task.output) if task.output else 0} characters")
    )
