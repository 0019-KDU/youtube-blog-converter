from flask import Blueprint, request, jsonify, render_template, redirect, url_for, session, send_file, g
...
from app.utils.validators import validate_youtube_url, extract_video_id, sanitize_filename
...
def generate_blog():
    ...
    try:
        ...
        logger.info(f"Blog generation started for user: {current_user['username']}, URL: {youtube_url}")
        ...
        try:
            ...
            logger.info(f"Blog content generated successfully: {len(blog_content)} characters")
        ...
        if not blog_content or len(blog_content) < 100:
            logger.error(f"Blog generation failed: Content too short or empty ({len(blog_content) if blog_content else 0} chars)")
        ...
        try:
            ...
            logger.error(f"Database error creating blog post: {str(db_error)}", exc_info=True)
            raise
        ...
        logger.error(f"Unexpected error during blog generation: {str(e)}", exc_info=True)
...
def dashboard():
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Database error retrieving posts: {str(db_error)}", exc_info=True)
            posts = []
...
def delete_post(post_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Database error deleting post: {str(db_error)}", exc_info=True)
            raise
...
def get_post(post_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Database error retrieving post: {str(db_error)}", exc_info=True)
            raise
...
def download_post_pdf(post_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Database error retrieving post for PDF: {str(db_error)}", exc_info=True)
            raise
        ...
        logger.error(f"PDF generation failed for post {post_id}: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
            'message': f'PDF generation failed: {str(e)}'
        }), 500
...
def contact():
    ...
    try:
        ...
        return render_template('error.html', error=f"Error loading contact page: {str(e)}"), 500