...
class RateLimiter:
    ...
    def _clean_old_entries(self, identifier, current_time):
        ...
        while (self.minute_buckets[identifier] and 
               self.minute_buckets[identifier][0] < minute_ago):
            self.minute_buckets[identifier].popleft()
        
        # Clean hour bucket
        while (self.hour_buckets[identifier] and 
               self.hour_buckets[identifier][0] < hour_ago):
    ...
    def get_remaining_requests(self, identifier=None):
        ...
        minute_remaining = max(0, self.requests_per_minute - len(self.minute_buckets[identifier]))
        hour_remaining = max(0, self.requests_per_hour - len(self.hour_buckets[identifier]))
