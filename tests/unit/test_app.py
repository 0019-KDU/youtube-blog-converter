...
def app():
    ...
    with patch.dict(
        ...
        with patch("app.load_dotenv"), patch("app.generate_blog_from_youtube"), patch(
            "app.PDFGeneratorTool"
        ), patch("app.User"), patch("app.BlogPost"), patch("app.auth_bp"), patch(
            "app.psutil"
        ), patch(
...
def mock_all_prometheus_metrics():
    ...
    with patch("app.blog_generation_requests") as mock_blog_requests, patch(
        "app.youtube_urls_processed"
    ) as mock_youtube_urls, patch("app.openai_tokens_used") as mock_tokens, patch(
        "app.blog_generation_duration"
    ) as mock_duration, patch(
        "app.database_operations"
    ) as mock_db_ops, patch(
        "app.blog_posts_created"
    ) as mock_blog_created, patch(
        "app.application_errors"
    ) as mock_app_errors, patch(
        "app.user_logins"
    ) as mock_user_logins, patch(
        "app.pdf_downloads"
    ) as mock_pdf_downloads, patch(
        "app.http_requests_total"
    ) as mock_http_requests, patch(
        "app.http_request_duration_seconds"
    ) as mock_http_duration, patch(
        "app.active_users"
...
class TestCleanupFunctions:
    ...
    def test_cleanup_after_generation(
        self, mock_cleanup_memory, mock_cleanup_com, mock_gc_collect
    ):
...
class TestUserManagement:
    ...
    @patch("app.User")
    def test_get_current_user_from_header(
        self, mock_user_class, mock_decode_token, request_context
    ):
    ...
    @patch("app.decode_token")
    @patch("app.User")
    def test_get_current_user_from_session_token(
        self, mock_user_class, mock_decode_token, request_context
    ):
        ...
        with patch.object(
            ...
            side_effect=lambda key, default=None: (
                "test-token" if key == "access_token" else default
            ),
        ):
            user = get_current_user()
    ...
    @patch("app.User")
    def test_get_current_user_from_session_user_id(
        self, mock_user_class, request_context
    ):
        ...
        with patch.object(
            ...
            "get",
            side_effect=lambda key, default=None: (
                "user123" if key == "user_id" else default
            ),
        ):
            user = get_current_user()
    ...
    def test_get_current_user_invalid_token(self, mock_decode_token, request_context):
        ...
        with patch.object(session, "get", return_value="invalid-token"), patch.object(
            session, "pop"
        ) as mock_pop:
...
class TestRoutes:
    ...
    def test_generate_blog_success(
        self,
        mock_cleanup,
        mock_store_large_data,
        mock_time,
        mock_extract_video_id,
        mock_blog_post_class,
        mock_generate_blog,
    ...
    @patch("app.get_current_user")
    def test_generate_blog_no_url(
        self, mock_get_current_user, client, mock_user, mock_all_prometheus_metrics
    ):
    ...
    def test_generate_blog_invalid_url(self, mock_get_current_user, client):
        ...
        with patch("app.blog_generation_requests") as mock_blog_requests:
            ...
            response = client.post(
                "/generate", data={"youtube_url": "https://www.google.com"}
            )
    ...
    @patch("app.get_current_user")
    @patch("app.extract_video_id")
    def test_generate_blog_invalid_video_id(
        self, mock_extract_video_id, mock_get_current_user, client
    ):
    ...
    @patch("app.extract_video_id")
    @patch("app.generate_blog_from_youtube")
    def test_generate_blog_generation_error(
        self, mock_generate_blog, mock_extract_video_id, mock_get_current_user, client
    ):
    ...
    def test_download_pdf_success(
        self, mock_pdf_tool_class, mock_retrieve_data, mock_get_current_user, client
    ):
        mock_get_current_user.return_value = {"_id": "user123", "username": "testuser"}
    ...
    @patch("app.get_current_user")
    @patch("app.retrieve_large_data")
    def test_download_pdf_no_data(
        self, mock_retrieve_data, mock_get_current_user, client
    ):
    ...
    def test_dashboard_success(
        self, mock_blog_post_class, mock_get_current_user, client
    ):
    ...
    @patch("app.BlogPost")
    @patch("app.PDFGeneratorTool")
    def test_download_post_pdf_success(
        self, mock_pdf_tool_class, mock_blog_post_class, mock_get_current_user, client
    ):
        mock_get_current_user.return_value = {"_id": "user123", "username": "testuser"}
...
class TestHealthMetricsEndpoint:
    ...
    @patch("app.time.time")
    def test_health_metrics_success_all_healthy(
        self, mock_time, mock_disk, mock_memory, mock_cpu, client
    ):
        """Test health metrics endpoint when all systems are healthy"""
        # Setup mocks
        mock_cpu.return_value = 25.5
        mock_memory.return_value = Mock(
            percent=60.2, used=8589934592, total=17179869184  # 8GB  # 16GB
        )
        mock_disk.return_value = Mock(
            used=107374182400, total=536870912000  # 100GB  # 500GB
        )
    ...
    @patch("app.psutil.disk_usage")
    @patch("app.time.time")
    def test_health_metrics_database_disconnected(
        self, mock_time, mock_disk, mock_memory, mock_cpu, client
    ):
        ...
        mock_memory.return_value = Mock(
            percent=80.5, used=12884901888, total=17179869184  # 12GB  # 16GB
        )
        mock_disk.return_value = Mock(
            used=268435456000, total=536870912000  # 250GB  # 500GB
        )
    ...
    @patch("app.psutil.virtual_memory")
    @patch("app.psutil.disk_usage")
    def test_health_metrics_no_start_time(
        self, mock_disk, mock_memory, mock_cpu, client
    ):
        """Test health metrics when app.start_time is not set"""
        # Setup mocks
        mock_cpu.return_value = 15.2
        mock_memory.return_value = Mock(
            percent=45.1, used=4294967296, total=17179869184  # 4GB  # 16GB
        )
        mock_disk.return_value = Mock(
            used=53687091200, total=536870912000  # 50GB  # 500GB
        )
    ...
    @patch("app.psutil.virtual_memory")
    @patch("app.psutil.disk_usage")
    def test_health_metrics_edge_case_disk_calculation(
        self, mock_disk, mock_memory, mock_cpu, client
    ):
    ...
    @patch("app.psutil.virtual_memory")
    @patch("app.logger")
    def test_health_metrics_disk_usage_error(
        self, mock_logger, mock_memory, mock_cpu, client
    ):
        ...
        with patch("app.application_errors") as mock_app_errors:
            ...
            with patch(
                "app.psutil.disk_usage", side_effect=OSError("Disk not accessible")
            ):
                response = client.get("/health-metrics")
    ...
    @patch("app.psutil.disk_usage")
    @patch("app.logger")
    def test_health_metrics_mongo_manager_import_error(
        self, mock_logger, mock_disk, mock_memory, mock_cpu, client
    ):
        ...
        with patch("app.application_errors") as mock_app_errors:
            ...
            with patch(
                "builtins.__import__",
                side_effect=ImportError("Cannot import auth.models"),
            ):
                response = client.get("/health-metrics")
    ...
    @patch("app.psutil.disk_usage")
    @patch("app.logger")
    def test_health_metrics_database_connection_error(
        self, mock_logger, mock_disk, mock_memory, mock_cpu, client
    ):
        ...
        with patch("app.application_errors") as mock_app_errors:
            ...
            with patch("auth.models.mongo_manager") as mock_mongo_manager:
                mock_mongo_manager.is_connected.side_effect = ConnectionError(
    ...
    @patch("app.psutil.virtual_memory")
    @patch("app.psutil.disk_usage")
    def test_health_metrics_large_temp_storage(
        self, mock_disk, mock_memory, mock_cpu, client
    ):
        ...
        client.application.temp_storage = {
            f"item_{i}": f"data_{i}" for i in range(1000)
        }
    ...
    @patch("app.psutil.virtual_memory")
    @patch("app.psutil.disk_usage")
    def test_health_metrics_missing_temp_storage(
        self, mock_disk, mock_memory, mock_cpu, client
    ):
    ...
    @patch("app.psutil.disk_usage")
    @patch("app.time.time")
    def test_health_metrics_response_format(
        self, mock_time, mock_disk, mock_memory, mock_cpu, client
    ):
    ...
    def test_health_metrics_track_requests_decorator(self, client):
        ...
        with patch("app.http_requests_total") as mock_requests_total, patch(
            "app.http_request_duration_seconds"
        ) as mock_duration, patch("app.psutil.cpu_percent", return_value=50.0), patch(
            "app.psutil.virtual_memory",
            return_value=Mock(percent=50.0, used=1000, total=2000),
        ), patch(
            "app.psutil.disk_usage", return_value=Mock(used=500, total=1000)
        ), patch(
            "auth.models.mongo_manager"
        ) as mock_mongo_manager:
    ...
    @patch("app.psutil.disk_usage")
    @patch("app.logger")
    def test_health_metrics_specific_exception_types(
        self, mock_logger, mock_disk, mock_memory, mock_cpu, client
    ):
        ...
        with patch("app.application_errors") as mock_app_errors:
            ...
            with patch("auth.models.mongo_manager") as mock_mongo_manager:
                mock_mongo_manager.is_connected.side_effect = ValueError(
    ...
    def test_health_metrics_content_type_header(self, client):
        ...
        with patch("app.psutil.cpu_percent", return_value=30.0), patch(
            "app.psutil.virtual_memory",
            return_value=Mock(percent=40.0, used=1500, total=4000),
        ), patch(
            "app.psutil.disk_usage", return_value=Mock(used=600, total=2000)
        ), patch(
            "auth.models.mongo_manager"
        ) as mock_mongo_manager:
...
class TestMiddleware:
    ...
    def test_log_request(self, app_context):
        ...
        with patch("app.uuid.uuid4") as mock_uuid, patch(
            "app.logger"
        ) as mock_logger, patch("time.time") as mock_time:
    ...
    def test_log_response(self, app_context):
        ...
        with patch("app.logger") as mock_logger, patch("time.time") as mock_time:
