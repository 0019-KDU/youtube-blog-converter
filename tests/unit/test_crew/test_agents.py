...
class TestAgents:
    ...
    def test_create_agents_success(
            self,
            mock_logger,
            mock_blog,
            mock_yt,
    ...
    @patch('app.services.blog_service.BlogGeneratorTool')
    @patch('app.crew.agents.logger')
    def test_agent_configuration_parameters(
            self, mock_logger, mock_blog, mock_yt, mock_agent):
    ...
    @patch('app.services.blog_service.BlogGeneratorTool')
    @patch('app.crew.agents.logger')
    def test_step_callbacks_functionality(
            self,
            mock_logger,
            mock_blog_tool,
            mock_youtube_tool,
    ...
    @patch('app.services.blog_service.BlogGeneratorTool')
    @patch('app.crew.agents.logger')
    def test_tool_initialization_failure(
            self,
            mock_logger,
            mock_blog_tool,
            mock_youtube_tool):
        ...
        mock_youtube_tool.side_effect = RuntimeError(
            "YouTube tool initialization failed")
    ...
    def test_agent_creation_failure(
        ...
        mock_agent_class.side_effect = ValueError(
...
class TestAgentsIntegration:
    ...
    def test_actual_agent_creation(self, mock_blog_tool, mock_youtube_tool):
        ...
        try:
            # Create proper mock tool instances that might satisfy pydantic
            # validation
            ...
            pytest.skip(
                f"CrewAI or tool validation not available for integration test: {e}")
