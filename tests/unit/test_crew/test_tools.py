...
"""
    ...
    def test_generate_pdf_bytes_output_exception_fallback(self, mock_fpdf_class):
        ...
        mock_pdf.output.side_effect = [Exception("dest not supported"), b'fallback_output']
    ...
    @patch('app.crew.tools.FPDF')
    @patch('app.crew.tools.gc')
    def test_generate_pdf_bytes_multipage_handling(self, mock_gc, mock_fpdf_class):
        ...
        # Note: _add_header_footer is not called in the mock, but we verify page_no() was checked
    ...
    @patch('app.crew.tools.FPDF')
    @patch('app.crew.tools.gc')
    def test_generate_pdf_bytes_cleanup_on_error(self, mock_gc, mock_fpdf_class):
...
"""
    ...
    def test_regex_patterns_in_content_processing(self):
        ...
        title_match = re.search(r"^#\s+(.+)$", content_with_title, re.MULTILINE)
