...
class TestAuthRoutes:
    ...
    @patch("auth.routes.get_current_user")
    def test_register_post_success_json(
        self, mock_get_user, mock_create_token, mock_user_class, client
    ):
    ...
    @patch("auth.routes.create_access_token")
    @patch("auth.routes.get_current_user")
    def test_register_post_success_form(
        self, mock_get_user, mock_create_token, mock_user_class, client
    ):
    ...
    @patch("auth.routes.render_template")
    @patch("auth.routes.get_current_user")
    def test_register_post_missing_fields_form(
        self, mock_get_user, mock_render, client
    ):
    ...
    @patch("auth.routes.create_access_token")
    @patch("auth.routes.get_current_user")
    def test_login_post_success_json(
        self, mock_get_user, mock_create_token, mock_user_class, client
    ):
    ...
    @patch("auth.routes.create_access_token")
    @patch("auth.routes.get_current_user")
    def test_login_post_success_form(
        self, mock_get_user, mock_create_token, mock_user_class, client
    ):
    ...
    def test_login_post_invalid_credentials(
        self, mock_get_user, mock_user_class, client
    ):
    ...
    def test_verify_token_success(self, mock_get_user, client):
        ...
        response = client.post(
            "/auth/verify-token", data=json.dumps({}), content_type="application/json"
        )
...
class TestAuthUtilityFunctions:
    ...
    def test_is_valid_email_invalid(self):
        ...
        for email in invalid_emails:
            assert (
                is_valid_email(email) is False
            ), f"Email '{email}' should be invalid but was validated as valid"
    ...
    def test_is_valid_password_valid(self):
        ...
        valid_passwords = ["password123", "12345678", "longpassword", "P@ssw0rd123"]
    ...
    def test_is_valid_password_invalid(self):
        ...
        invalid_passwords = ["short", "1234567", "", "pass"]  # 7 characters
    ...
    def test_get_current_user_with_bearer_token(
        self, mock_user_class, mock_decode_token, client
    ):
