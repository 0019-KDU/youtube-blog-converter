...
class TestMongoDBConnectionManager:
    ...
    @patch.dict(
        "os.environ",
        {"MONGODB_URI": "mongodb://test:27017", "MONGODB_DB_NAME": "test_db"},
    )
    ...
    def test_connect_no_uri(self, mock_mongo_client):
        ...
        with pytest.raises(
            ValueError, match="MONGODB_URI environment variable not set"
        ):
            manager._connect()
...
class TestUser:
    ...
    def test_update_user_success(self, mock_manager, sample_user_data):
        ...
        result = user_model.update_user(
            sample_user_data["_id"], {"username": "newusername"}
        )
...
class TestBlogPost:
    ...
    def test_delete_post_success(self, mock_manager, sample_blog_post):
        ...
        blog_model = BlogPost()
        result = blog_model.delete_post(
            sample_blog_post["_id"], sample_blog_post["user_id"]
        )
