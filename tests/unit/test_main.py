...
class TestMainFunctions(unittest.TestCase):
    ...
    def test_extract_video_id_valid_special_characters(self):
        ...
        self.assertEqual(
            result, "abc-def_ghi", "Valid 11-character ID should be accepted"
        )
    ...
    def test_individual_components_test_success(
        self, mock_blog_class, mock_transcript_class
    ):
    ...
    @patch("src.tool.YouTubeTranscriptTool")
    @patch("src.tool.BlogGeneratorTool")
    def test_individual_components_test_blog_fail(
        self, mock_blog_class, mock_transcript_class
    ):
    ...
    @patch("src.tool.YouTubeTranscriptTool")
    @patch("src.tool.BlogGeneratorTool")
    def test_individual_components_test_import_error(
        self, mock_blog_class, mock_transcript_class
    ):
    ...
    @patch.dict(
        os.environ, {"OPENAI_API_KEY": "test", "SUPADATA_API_KEY": "test"}, clear=True
    )
    ...
    def test_generate_blog_url_validation_edge_cases(self):
        ...
        for url in working_urls:
            with patch("src.main._extract_video_id", return_value="test123"):
                with patch(
                    "src.main.individual_components_test", return_value="content" * 200
                ):
                    with patch.dict(
                        os.environ,
                        {"OPENAI_API_KEY": "test", "SUPADATA_API_KEY": "test"},
                        clear=True,
                    ):
                        result = generate_blog_from_youtube(url)
                        # Should not contain error messages for valid URLs
                        self.assertNotIn(
                            "Invalid YouTube URL", result, f"URL {url} should be valid"
                        )
        ...
        for url in failing_urls:
            with patch.dict(
                ...
                self.assertIn(
                    "Invalid YouTube URL",
                    result,
                    f"URL {url} currently fails due to regex limitation",
                )
    ...
    def test_transcript_exactly_100_chars(self):
        ...
        with patch("src.tool.YouTubeTranscriptTool") as mock_transcript_class:
            mock_transcript_instance = Mock()
            mock_transcript_instance._run.return_value = (
                "a" * 100
    ...
    @patch("builtins.input", side_effect=["https://youtu.be/abc", "en"])
    @patch(
        "src.main.validate_environment",
        side_effect=RuntimeError("Missing environment variables"),
    )
    ...
    def test_cli_main_timestamp_filename(self, mock_time, mock_generate, mock_input):
        ...
        mock_open.assert_called_once_with(
            "blog_output_1234567890.txt", "w", encoding="utf-8"
        )
