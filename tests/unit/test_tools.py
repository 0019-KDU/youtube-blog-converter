    ...
    def test_log_error_on_missing_supadata_key(self):
        ...
        with patch("src.tool.SUPADATA_API_KEY", None):
            with patch("src.tool.logger") as mock_logger:
                with pytest.raises(
                    RuntimeError, match="Supadata API key not configured"
                ):
                    YouTubeTranscriptTool()
...
class TestPDFGeneratorTool:
    ...
    def test_clean_unicode_text_unicode_chars(self):
        ...
        text_with_unicode = (
            "Text with em\u2014dash and \u201cquotes\u201d and \u2026ellipsis"
        )
    ...
    @patch("src.tool.FPDF")
    @patch("src.tool.gc.collect")
    def test_generate_pdf_bytes_success(
        self, mock_gc_collect, mock_fpdf_class, sample_blog_content
    ):
    ...
    @patch("src.tool.FPDF")
    @patch("src.tool.gc.collect")
    def test_generate_pdf_bytes_cleanup_on_exception(
        self, mock_gc_collect, mock_fpdf_class
    ):
        """Test cleanup is called even when exception occurs"""
    ...
    @patch("src.tool.FPDF")
    @patch("src.tool.gc.collect")
    def test_generate_pdf_bytes_pdf_none_cleanup(
        self, mock_gc_collect, mock_fpdf_class
    ):
    ...
    def test_generate_pdf_bytes_long_title_multiline(self, mock_fpdf_class):
        ...
        mock_pdf.r_margin = 15
        mock_pdf.get_y.return_value = 50
        mock_pdf.get_string_width.side_effect = (
...
This is a long paragraph that should be justified when rendered in the PDF. It contains enough text to span multiple lines and demonstrate the justification feature."""
    ...
    def test_generate_pdf_bytes_output_exception_fallback(self, mock_fpdf_class):
        ...
        mock_pdf.output.side_effect = [
            Exception("Output failed"),
            b"fallback pdf content",
        ]
        mock_fpdf_class.return_value = mock_pdf
    ...
    def test_generate_pdf_with_metadata(self):
        ...
        with patch.object(PDFGeneratorTool, "generate_pdf_bytes") as mock_generate:
            ...
            result = tool.generate_pdf_with_metadata(
                "# Test Content", title="Test Title", author="Test Author"
            )
    ...
    def test_generate_pdf_with_metadata_logging(self, mock_logger):
        ...
        with patch.object(PDFGeneratorTool, "generate_pdf_bytes") as mock_generate:
            ...
            mock_logger.error.assert_called_once()
            assert "PDF generation with metadata failed" in str(
...
class TestCleanupResources:
    ...
    def test_cleanup_resources_with_set_debug(self):
        ...
        with patch("src.tool.gc.collect") as mock_collect, patch(
            "src.tool.gc.set_debug"
        ) as mock_set_debug:
            cleanup_resources()
